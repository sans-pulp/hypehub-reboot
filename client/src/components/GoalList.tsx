 ...
 */
export const GoalListContainer = ({ goals, profileId, onGoalComplete }: GoalListContainerProps) => {
...
const GoalCategory = ({ goals, title, loading, completedGoalId, onComplete }: GoalCategoryProps) => {
                ...
                {goals.map(goal => (
                    <li key={goal.id} className="mb-2">
                        <div className="flex justify-between items-center">
                            ...
                            <button 
                                className={`nes-btn is-success ${loading && completedGoalId === goal.id ? 'is-disabled' : ''}`}
                                onClick={() => onComplete(goal.id)}
                                disabled={loading && completedGoalId === goal.id}
                            >
                                {loading && completedGoalId === goal.id ? 'Saving...' : 'Complete'}
                            </button>
                        </div>
                    </li>
                ))}
